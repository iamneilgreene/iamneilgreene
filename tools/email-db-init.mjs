...
const sql = postgres(url, { ssl: { rejectUnauthorized: true, ...(process.env.EMAIL_DATABASE_CA_BASE64 ? { ca: Buffer.from(process.env.EMAIL_DATABASE_CA_BASE64, 'base64').toString('utf8') } : {}) }, max: 1, connect_timeout: 8, prepare: false, onnotice: () => {} })
try {
  await sql.unsafe(await readFile(new URL('../supabase/migrations/20260916020024_secure_email_tables.sql', import.meta.url), 'utf8'))
