import PageContainer from '@/components/layout/PageContainer'
import Button from '@/components/ui/Button'
import Link from 'next/link'

// TODO: Wire to Blueprint email opt-in form or landing page when ready
const BLUEPRINT_HREF = '#blueprint'

export default function ResourcesHeroSection() {
  return (
    <section className="relative bg-[#0d0d0d] pt-40 pb-24 md:pt-48 md:pb-32">
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#b8975a]/40 to-transparent" />

      <PageContainer width="narrow">
        <div className="flex flex-col gap-8">
          <span className="text-[0.7rem] font-semibold uppercase tracking-[0.25em] text-[#b8975a]">
            Resources
          </span>

          <h1 className="font-display text-4xl font-semibold leading-tight tracking-tight text-[#f4f1ec] md:text-5xl lg:text-6xl">
            Practical Tools for Men Who Need a Stronger Starting Point
          </h1>

          <span className="block h-px w-14 bg-[#b8975a]" />

          <p className="text-base leading-relaxed text-[#9a9590] md:text-lg">
            These resources are built for men who need clarity, structure, and a practical first
            step. Not more noise. Not more theory. Just tools that help rebuild discipline,
            momentum, and stronger standards in real life.
          </p>

          <div className="flex flex-col gap-4 pt-2">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              <Button href={BLUEPRINT_HREF} variant="primary" size="lg">
                Get the Blueprint
              </Button>
              <Button href="/book" variant="gold-outline" size="lg">
                Get the Book
              </Button>
            </div>
            <div className="pt-1">
              <Link
                href="/community"
                className="text-sm text-[#9a9590] underline underline-offset-4 hover:text-[#b8975a] transition-colors duration-200"
              >
                Join the Brotherhood
              </Link>
            </div>
          </div>
        </div>
      </PageContainer>
    </section>
  )
}
