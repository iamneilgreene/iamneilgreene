        ...
        <form className="mt-6 space-y-5" onSubmit={async (event) => {
          ...
          try {
            const response = await fetch('/api/profile-subscription', {
              ...
              body: JSON.stringify({ name: data.get('name'), email: data.get('email'), consent: data.get('consent') === 'on', website: data.get('website'), profile }),
            ...
            setReceipt('Your profile email was accepted for delivery. Check your inbox and spam folder. No automatic reminder is scheduled.' + (data.get('consent') === 'on' && !result.consentSaved ? ' Your educational-email permission could not be saved; request updates later through the contact page.' : ''))
            setState('saved')
          } catch (failure) {
          ...
          <label className="flex items-start gap-3 text-sm leading-relaxed text-text-body">
            ...
            <span>Optional: I agree to receive educational emails from Neil Greene. I can withdraw permission through the <Link href="/contact?reason=other" className="underline underline-offset-4">contact page</Link>.</span>
          </label>
