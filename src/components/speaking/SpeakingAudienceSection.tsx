    ...
    <section className="relative bg-[#0d0d0d] py-20 md:py-28">
      ...
      <PageContainer>
        <div className="flex flex-col gap-12">
          ...
          <div className="grid gap-px border border-[#2e2e2e] md:grid-cols-2">
            {audiences.map((audience) => (
              <div
                key={audience.title}
                className="flex flex-col gap-4 bg-[#0d0d0d] p-8"
              >
                <h3 className="font-display text-lg font-semibold text-[#f4f1ec]">
                  {audience.title}
                </h3>
                <p className="text-sm leading-relaxed text-[#9a9590]">{audience.body}</p>
              </div>
            ))}
          </div>
