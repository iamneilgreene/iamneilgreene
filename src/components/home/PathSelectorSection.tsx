    ...
    <section className="bg-[#eae5dc] py-20 md:py-28">
      <PageContainer>
        <div className="flex flex-col gap-12">
          ...
          {/* Cards */}
          <div className="grid gap-0 border border-[#cfc8bb] md:grid-cols-3">
            {paths.map((path, i) => (
              <div
                key={path.title}
                className={`flex flex-col gap-6 p-8 transition-colors duration-200 hover:bg-[#ddd7cc] ${
                  i < 2 ? 'border-b border-[#cfc8bb] md:border-b-0 md:border-r' : ''
                }`}
              >
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <p className="text-sm leading-relaxed text-[#6b6158]">{path.copy}</p>
                </div>

                <Button href={path.href} variant="ghost" size="sm" className="mt-auto self-start px-0 text-[#6b6158] hover:bg-transparent hover:text-[#9a7d45]">
                  {path.cta} &rarr;
                </Button>
              </div>
            ))}
