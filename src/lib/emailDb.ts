...
export function emailDb() {
  ...
  connection ??= postgres(url, { ssl: { rejectUnauthorized: true, ...(process.env.EMAIL_DATABASE_CA_BASE64 ? { ca: Buffer.from(process.env.EMAIL_DATABASE_CA_BASE64, 'base64').toString('utf8') } : {}) }, max: 3, idle_timeout: 20, connect_timeout: 8, prepare: false, onnotice: () => {}, connection: { statement_timeout: 10000, idle_in_transaction_session_timeout: 60000 } })
  return connection
}
